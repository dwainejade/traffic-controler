...
export function SignalHeads({ level, world }: { level: LevelDef; world: World }) {
  ...
  useLayoutEffect(() => {
    ...
    heads.forEach((head, i) => {
      ...
      m.compose(
        new THREE.Vector3(head.foot.x, 2.6, head.foot.z),
        q,
        one,
      )
  ...
  useFrame(() => {
    ...
    heads.forEach((head, i) => {
      ...
      const mine =
        head.connectors.length > 0 && head.connectors.every((c) => serving.includes(c))
      const state =
      ...
      const seconds =
        state === 'green'
          ...
          : timeToNextGreen(junction, world.programOf(junction), (p) =>
              junction.phases[p].connectors.some((c) => head.connectors.includes(c)),
            )
      ...
      {heads.map((head, i) =>
        [1, -1].map((face) => (
          <Text
             ...
             */
            position={[
              head.panel.x + Math.sin(head.facing) * face * (PANEL_D / 2 + 0.02),
              head.panel.y,
              head.panel.z + Math.cos(head.facing) * face * (PANEL_D / 2 + 0.02),
            ]}
