...
function bladeLength(name: string): number {
  return THREE.MathUtils.clamp(name.length * 0.1 + 0.6, 2.4, 6);
}
...
function buildSigns(level: LevelDef): Sign[] {
  ...
  for (const junction of junctionArms(level)) {
    ...
    for (const [i, nose] of noses.entries()) {
      ...
      const streets =
        named.length === 2 && named[0].name === named[1].name
          ? [named[0]]
          : named;
...
function bladeAngle(arm: Arm, other: Arm): number {
  ...
  return faceNormal(angle).x * other.dir.x +
    faceNormal(angle).z * other.dir.z >=
    0
    ? angle
    : angle + Math.PI;
}
...
export function StreetSigns({ level }: { level: LevelDef }) {
  ...
  useLayoutEffect(() => {
    ...
    signs.forEach((sign, i) => {
      m.compose(
        new THREE.Vector3(sign.x, POST_HEIGHT / 2, sign.z),
  ...
  return (
    <group>
      <instancedMesh
        ref={posts}
        args={[POST_GEOM, undefined, signs.length]}
        castShadow
      >
      ...
      <instancedMesh
        ref={plates}
        args={[BLADE_GEOM, undefined, blades.length]}
        castShadow
      >
